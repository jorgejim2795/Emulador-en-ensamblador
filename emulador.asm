...
section .data
	...
	len3: equ $-int3
	msjfun db "funcion invalido linea: "
	lmsjfun equ $-msjfun
	msjopcode db "opcode invalido linea: "
	lmsjopcode equ $-msjopcode
	msjdrinv db "direccion de memoria invalida linea: "
	lmsjdrinv: equ $-msjdrinv
	
	stackdrinv db "direccion de memoria fuera del stack linea: "
	lstackdrinv: equ $-stackdrinv

	msjinvcar db "caracter no valido en la ROM",0xa
	lmsjinvcar: equ $-msjinvcar
	Exitosa db "Ejecución Exitosa",0xa
	...
	argrande db "El argumento es mayor a 4 digitos",0xa
...
section .text
	global main
main:	
;PROCESO PARA INICIALIZAR LOS ARGUMENTOS
;----------------------------------------------------------------------
	mov r8,0
_REGLOP:
	mov dword [reg+r8],0d	
	...
	mov r8,0
	mov r9,0
;PROCESO PARA SACAR LOS ARGUMENTOS 
;----------------------------------------------------------------------  
      	push    rbp                                     ; 0000 _ 55
	...
	mov     rax, qword [rbp-10H]                    ; 0023 _ 48: 8B. 45, F0
        mov     rax, qword [rax]                        ; 0027 _ 48: 8B. 00
       
	cmp r14,1
	je aqui
        
	mov     rax, qword [rbp-10H]                    ; 003C _ 48: 8B. 45, F0
        add     rax, 8                                  ; 0040 _ 48: 83. C0, 08
        mov     rax, qword [rax]                        ; 0044 _ 48: 8B. 00
	mov 	r11, [rax]
	mov 	qword [arg0], r11
	cmp 	r14,2
	je 	cargA0
        ...
        add     rax, 16                                 ; 005D _ 48: 83. C0, 10
        mov     rax, qword [rax]                        ; 0061 _ 48: 8B. 00
  	mov 	r11, [rax]
	mov 	qword [arg1], r11	
	cmp 	r14,3
	je 	cargA1
        ...
        add     rax, 24                                 ; 007A _ 48: 83. C0, 18
        mov     rax, qword [rax]                        ; 007E _ 48: 8B. 00
	mov 	r11, [rax]
	mov 	qword [arg2],r11
	cmp 	r14,4
	je 	cargA2
	...
	mov     rax, qword [rbp-10H]                    ; 0059 _ 48: 8B. 45, F0
        add     rax, 32	                            	; 005D _ 48: 83. C0, 10
        mov     rax, qword [rax]                        ; 0061 _ 48: 8B. 00
	mov 	r11,[rax]
	mov 	qword [arg3],r11

;PROCESO PARA DECODIFICAR LOS ARGUMENTOS DE ASCII A DECIMAL 
;----------------------------------------------------------------------------
...
or31:
	mov r10,[arg3]
	and r10,000000000000000ffh
	shl r10, 24
	mov qword [arg3], r10
	jmp unidad3
or32:
	mov r10,[arg3]
	and r10,0000000000000ffffh
	shl r10, 16
	mov qword [arg3], r10
	jmp decena3
...
aqui:
	mov r8d, dword [reg+16]
	mov r9d, dword [reg+20]
	mov r10d, dword [reg+24]
	mov r12d, dword [reg+28]
here:
...
invopcode:
	...
	jmp _salir
;-------------------------------------------------
;mensaje indica ejecucion fallida
;-------------------------------------------------
_ejfa:
...
;-------------------------------------------------
;mensaje indica direccion de memoria fuera del stack
;-------------------------------------------------
...
_register:
	...
	mov rdx, 3
	syscall
; COMPARA CUAL ES EL REGISTRO Y LO IMPRIME EN PANTALLA
;--------------------------------------------------------------------------------
	cmp r15,0
	je _at
	cmp r15,4
...
; COMPARA CUAL ES EL REGISTRO Y LO IMPRIME EN EL TXT
;--------------------------------------------------------------------------------
_at:
	Wfile regis,3
	ret
...
;----------------------------------------------
	Wfile enteer,1
;EJECUTA LA OR ENTRE LOS DOS REGISTROS
;---------------------------------------------------
...
;-----------------------imprime la instrucion
	...
	Wfile coma,1
	call rt						;Llamo a RT
;HACE LA OR ENTRE LOS DOS REGISTROS Y LO NIEGA PARA TENER LA NOR
;---------------------------------------------------------------------------
...
sub:
;-----------------------imprime la instrucion ESCRIBE EN EL TXT Y OBTINE LOS DATOS DE LOS REGISTROS
	mov rdi, 1					; sys write	
	mov rax, 1
	mov rsi, instruc+82
	mov rdx, 4
...
;HACE LA RESTA CON LOS CONTENIDOS DE LOS REGISTROS
;-------------------------------------------------------------
	sub r12,r13
...
;HACE LA SUMA ENTRE EL REGISTRO Y EL IMMEDIATE
;--------------------------------------------------------------------------------
...
;HACE EL AND ENTRE EL REGISTRO Y EL IMMEDIATE
;--------------------------------------------------------------
...
;HACE EL OR ENTRE EL REGISTRO Y EL IMMEDIATE
...
;-----------------------imprime la instrucion
	...
	Wfile instruc+96,5
	call rd						;Llamo a RD
	mov r10d,[rmflo]				;muevo el registro LOW de la multiplicacion  a RD
	mov dword[reg+r11d], r10d 
	mov rdi,1	
	mov rax, 1
	mov rsi, enteer
	mov rdx, 1
...
;-----------------------imprime la instrucion
	...
	cmp r13, 0d					;compara con la mayor direccion 
	jl drinv					;inidca que la direccion no es valida
...
;-----------------------imprime la instrucion	
	...
	jge drinv					;inidca que la direccion no es valida
	cmp r13d, 0					;compara con la mayor direccion 
	jl drinv					;inidca que la direccion no es valida
...
;IMPRIME LOS VALORES DE LOS RESGITROS AL TERMINAR LA EXECUCION DEL CODIGO
;-----------------------------------------------------------------------------------
